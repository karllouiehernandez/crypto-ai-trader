...
2. Before changing a parameter, check `parameter_history.md` to avoid re-testing known bad values
3. Before a backtest, check `backtest_results/` for prior runs on the same symbol/period
4. Sprint cannot close until code review sub-agent approves — see `sprint_log.md`
